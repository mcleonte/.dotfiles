...
local load = function(path)
	...
	local _ok, res = pcall(require, "plugins.config." .. path)
	if _ok then
		return res
	else
...
local use = function(config)
	if config.ext then
		...
		config = vim.tbl_deep_extend("force", config, { config = config_ext })
		config.ext = nil
	end
	packer.use(config)
end
...
local function packer_startup(func)
	packer.startup({ func, config = {} })
	install_sync()
end

return { packer_startup, use }
